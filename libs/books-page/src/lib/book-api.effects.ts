...
export class BooksApiEffects {
  ...
  createBook$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(BooksPageActions.createAbook),
      concatMap((action) => {
        return this.booksService
          .create(action.book)
  ...
  updateBook$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(BooksPageActions.updateAbook),
      concatMap((action) => {
        return this.booksService
  ...
  deleteBook$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(BooksPageActions.deleteAbook),
      concatMap((action) => {
        return this.booksService
          .delete(action.bookId)
          .pipe(
            map(() => BooksApiActions.booksDeleted({ bookId: action.bookId }))
          ); //
      })
    );
  });
